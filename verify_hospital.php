...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sqlcheck = "SELECT * FROM hospital WHERE email='" . $email . "' AND idno='" . $idno . "';";
    $result = mysqli_query($conn, $sqlcheck);
    $user = mysqli_fetch_assoc($result);
    mysqli_free_result($result);

    if ($user == NULL) {
        $userPresentErr = "NO SUCH HOSPITAL RECORD EXISTS WITH THESE DETAILS";
    } else {
        header("Location: change_hospital_password.php?H_id=" . $user['H_id']);
    }
}
?>
<!DOCTYPE html>
...
<body>
    <div class="container">
        ...
        <div class="login">
            <h2 class="pgHeading"><i class="fas fa-key"></i>&nbsp;RESET HOSPITAL LOGIN PASSWORD PORTAL</h2>
